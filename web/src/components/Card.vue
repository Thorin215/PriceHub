<template>
    <el-scrollbar height="100%" style="width: 100%;">
        ...
        :style="{ 'background-image': card.type === 'Student' ? 'url(https://blog-pic-thorin.oss-cn-hangzhou.aliyuncs.com/9862f0ea16d85e533cf929567f7e2b7.png)' : 'url(https://blog-pic-thorin.oss-cn-hangzhou.aliyuncs.com/8fde159d6865608bde613ca860b5b20.png)' }">
                <div>
                    <!-- 卡片标题 -->
                    <div style="font-size: 15px; font-weight: bold;">No. {{ card.id }}</div>

                    <el-divider />

                    <!-- 卡片内容 -->
                    <div style="margin-left: 8px; text-align: start; font-size: 16px;">
...
export default {
    ...
    methods: {
        ...
        ConfirmModifyCard() {
            ...
            .catch(error => {
                console.error("修改借书证信息失败:", error);
                ElMessage.error("借书证信息修改失败，请稍后重试");
            });
        },
        ConfirmRemoveCard() {
            ...
            .catch(error => {
                console.error("删除借书证失败:", error);
                ElMessage.error("借书证删除失败，请稍后重试");
            });
        },
        QueryCards() {
            this.cards = [] // 清空列表
