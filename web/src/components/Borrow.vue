...
export default {
    ...
    this.tableData = []; // 清空列表
    try {
        let response = await axios.post('/borrow', { cardID: this.toQuery }); // 向 /borrow 发出 POST 请求，请求体包含 cardID=this.toQuery
        let borrows = response.data; // 获取响应数据
        borrows.forEach(borrow => { // 对于每一个借书记录
            this.tableData.push(borrow); // 将其加入到列表中
        });
        this.isShow = true; // 显示结果列表
    } catch (error) {
